            ...
            <tbody>
              {items.map((it) => (
                <tr key={it._id} style={{ borderTop: "1px solid #eee" }}>
                  ...
                  <Td dataLabel="Price">₹{it.price}</Td>
                  ...
                  <Td dataLabel="Total">₹{(it.price || 0) * (it.qty || 1)}</Td>

                  <Td dataLabel="Action">
                    <button onClick={() => dispatch(removeFromCart(it._id))} style={btn("delete")}>
                      Remove
                    </button>
                  </Td>
                </tr>
...
// ✅ accepts dataLabel for mobile stacked labels
