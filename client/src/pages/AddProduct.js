...
export default function AddProduct() {
  const [form, setForm] = useState({
    name: "", price: "", category: "", stock: "", description: "", brand: ""
  });
  ...
  const onFiles = (e) => {
    const arr = Array.from(e.target.files || [])
      .filter((f) => f.type.startsWith("image/"))  // only images
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      files.forEach((f) => fd.append("images", f)); // field "images" matches backend
